...
class LeggedRobot(BaseTask):
    ...
    def check_termination(self):
        ...
        self.delta_z = l_align*torch.sin(p_align) + 0.8 - self.base_pos[:, 2]
    ...
    def get_lpy_in_base_coord(self, env_ids):
        ...
        z = torch.mean(self.grasper_in_world[:, 2].unsqueeze(1) - self.measured_heights, dim=1) - 0.8  # Updated base height offset from 0.38 to 0.8
    ...
    def _lpy_to_world_xyz(self, env_ids):
        ...
        z_ = z + self.measured_heights + 0.8  # Updated base height offset from 0.38 to 0.8
        return x_, y_, z_
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        if self.cfg.env.observe_clock_inputs:
            # clock_inputs_sin and clock_inputs_cos are each 1 scalar value
            noise_vec = torch.cat((noise_vec, torch.zeros(2)), dim=0)
        if self.cfg.env.observe_vel:
        ...
        if self.cfg.env.observe_contact_states:
            # Use len(self.feet_indices) instead of hardcoded 4 for biped robots (2 feet)
            num_feet = len(self.feet_indices) if hasattr(self, 'feet_indices') else 2
            noise_vec = torch.cat((noise_vec,
                                   torch.ones(num_feet) * noise_scales.contact_states * noise_level,
                                   ), dim=0)
    ...
    def _init_buffers(self):
        ...
        # Foot heights (for reward functions)
        self.foot_heights = torch.zeros(self.num_envs, len(self.feet_indices), dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _init_custom_buffers__(self):
        ...
        self.friction_coeffs = self.default_friction * torch.ones(self.num_envs, 2, dtype=torch.float, device=self.device,
                                                                  requires_grad=False)
        self.restitutions = self.default_restitution * torch.ones(self.num_envs, 2, dtype=torch.float, device=self.device,
                                                                  requires_grad=False)
    ...
    def _parse_cfg(self, cfg):
        ...
        self.gaits_ranges = vars(self.cfg.gait.ranges)  # Fixed: used vars() instead of undefined class_to_dict
