...
from go1_gym.envs.automatic.limx_robot_config import Cfg
...
def config_wtw(Cnfg: Union[Cfg, Meta]):
    ...
    Cnfg.domain_rand.lag_timesteps = 6  # solefoot: delay_ms_range [0,20] → average ~10ms = 4 steps
    Cnfg.domain_rand.randomize_lag_timesteps = True  # solefoot: randomize_action_delay = True
    ...
    Cnfg.domain_rand.friction_range = [0.0, 1.6]  # Updated to match solefoot_flat
    Cnfg.domain_rand.randomize_restitution = True
    Cnfg.env.priv_observe_restitution = True
    Cnfg.domain_rand.restitution_range = [0.0, 1.0]  # Updated to match solefoot_flat
    Cnfg.domain_rand.randomize_base_mass = True
    Cnfg.env.priv_observe_base_mass = False
    Cnfg.domain_rand.added_mass_range = [-0.5, 5]  # Updated to match solefoot_flat
    Cnfg.domain_rand.randomize_gravity = False  # Disabled in solefoot_flat
    ...
    Cnfg.domain_rand.randomize_com_displacement = True  # Enabled in solefoot_flat (as randomize_base_com)
    Cnfg.domain_rand.com_displacement_range = [-0.03, 0.03]  # Updated to match solefoot_flat rand_com_vec
    Cnfg.env.priv_observe_com_displacement = False
    Cnfg.domain_rand.randomize_inertia = True  # Added from solefoot_flat
    Cnfg.domain_rand.randomize_inertia_range = [0.8, 1.2]  # Added from solefoot_flat
    Cnfg.domain_rand.randomize_ground_friction = False  # Disabled (solefoot doesn't use this)
    ...
    Cnfg.domain_rand.randomize_motor_strength = True  # solefoot: randomize_motor_torque
    Cnfg.domain_rand.motor_strength_range = [0.8, 1.2]  # Updated to match solefoot_flat motor_torque_range
    Cnfg.env.priv_observe_motor_strength = False
    Cnfg.domain_rand.randomize_motor_offset = True  # solefoot: randomize_default_dof_pos
    Cnfg.domain_rand.motor_offset_range = [-0.05, 0.05]  # Updated to match solefoot_flat default_dof_pos_range
    Cnfg.env.priv_observe_motor_offset = False
    Cnfg.domain_rand.push_robots = False  # Enabled in solefoot_flat
    Cnfg.domain_rand.push_interval_s = 7  # Added to match solefoot_flat
    Cnfg.domain_rand.max_push_vel_xy = 1.0  # Added to match solefoot_flat
    Cnfg.domain_rand.randomize_Kp_factor = True  # Enabled in solefoot_flat (as randomize_Kp)
    Cnfg.domain_rand.Kp_factor_range = [0.8, 1.2]  # Updated to match solefoot_flat
    Cnfg.env.priv_observe_Kp_factor = False
    Cnfg.domain_rand.randomize_Kd_factor = True  # Enabled in solefoot_flat (as randomize_Kd)
    Cnfg.domain_rand.Kd_factor_range = [0.8, 1.2]  # Updated to match solefoot_flat
    ...
    # Cnfg.rewards.base_height_target = 0.75  # Updated to match solefoot_flat
    ...
    # Cnfg.rewards.kappa_gait_probs = 0.05  # Updated to match solefoot_flat
    # Cnfg.rewards.gait_force_sigma = 25.0  # Updated to match solefoot_flat
    # Cnfg.rewards.gait_vel_sigma = 0.25  # Updated to match solefoot_flat
    # Cnfg.rewards.gait_height_sigma = 0.005  # Added to match solefoot_flat
    ...
    Cnfg.commands.lin_vel_x = [-1.0, 1.5]  # Updated to match solefoot_flat
    Cnfg.commands.lin_vel_y = [-1.0, 1.0]  # Updated to match solefoot_flat
    Cnfg.commands.ang_vel_yaw = [-1, 1]
    Cnfg.commands.body_height_cmd = [0.68, 0.78]  # Updated to match solefoot_flat absolute height range
    Cnfg.commands.gait_frequency_cmd_range = [1.0, 1.5]  # Updated to match solefoot_flat
    Cnfg.commands.gait_phase_cmd_range = [0.0, 1.0]
    Cnfg.commands.gait_offset_cmd_range = [0.5, 0.5]  # Updated to match solefoot_flat (fixed offset)
    ...
    Cnfg.commands.limit_vel_x = [-1.5, 1.5]  # Updated to match solefoot_flat velocity range
    Cnfg.commands.limit_vel_y = [-1.0, 1.0]  # Updated to match solefoot_flat
    Cnfg.commands.limit_vel_yaw = [-1.0, 1.0]  # Updated to match solefoot_flat
    Cnfg.commands.limit_body_height = [0.68, 0.78]  # Updated to match solefoot_flat
    Cnfg.commands.limit_gait_frequency = [1.0, 1.5]  # Updated to match solefoot_flat
    Cnfg.commands.limit_gait_phase = [0.0, 1.0]
    Cnfg.commands.limit_gait_offset = [0.5, 0.5]  # Updated to match solefoot_flat
    ...
    Cnfg.normalization.friction_range = [0.0, 1.6]
    Cnfg.normalization.ground_friction_range = [0.0, 1.6]
